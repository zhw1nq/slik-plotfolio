...
<template>
  <component
    ...
    :class="[
      ...
      tight ? 'p-1.5 sm:p-2' : 'p-3 sm:p-4',
      {
        'cursor-pointer': cursor === true,
        'items-center flex space-x-3 sm:space-x-4': $slots.icon || $slots['icon-left'],
  ...
  >
    ...
    <div class="overflow-x-hidden leading-relaxed space-y-1.5 sm:space-y-2 min-w-0">
      <h2
        v-if="$slots.title"
        class="font-medium text-black dark:text-white truncate text-sm sm:text-base"
      >
        <slot
          name="title"
      ...
      <p
        v-if="$slots.default"
        class="text-black/50 dark:text-white/30 text-xs sm:text-sm"
