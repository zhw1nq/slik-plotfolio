...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-neutral-900">
    <Navbar class="pt-6 sm:pt-10" />

    <!-- Nuxt component -->
    <main class="responsive-screen min-h-screen pb-4 sm:pb-8">
      <div class="space-y-3 sm:space-y-4 py-4 sm:py-8">
        <div class="space-y-2">
          <h1 class="text-black/90 dark:text-white/90 font-semibold text-xl sm:text-2xl">
            An error occured
          </h1>

          <p class="text-base sm:text-lg text-black/50 dark:text-white/30">
            Here are the details:
          </p>
        </div>

        <div class="rounded card-base gap-4 sm:gap-6 flex flex-col md:flex-row">
          <SmartImage
            :src="`https://http.cat/${error?.statusCode}.jpg`"
            class="rounded w-full md:w-80 flex-shrink-0"
          />

          <div class="space-y-4 sm:space-y-6 whitespace-normal">
            <div class="flex flex-col space-y-1.5 sm:space-y-2">
              <PageTitle> Title </PageTitle>
              <span class="text-black/50 dark:text-white/30 text-sm sm:text-base">{{
                getErrorMeta.title
              }}</span>
            </div>
            ...
            <div class="flex flex-col space-y-1.5 sm:space-y-2">
              ...
              <pre
                class="whitespace-pre-wrap text-black/50 dark:text-white/30 text-xs sm:text-sm overflow-x-auto"
        ...
        <div
          class="flex items-center justify-center sm:justify-between md:justify-start flex-wrap gap-3 sm:gap-4"
        >
